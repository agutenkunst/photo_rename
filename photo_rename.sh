...
pixel_modes_match2="(?:IMG_|VID_|MVIMG_|PANO_|PXL_|)${pixel_date_capture}_${pixel_time_capture}(?P<stuff>.*)"
...
replace_datetime='$+{year}-$+{month}-$+{day}T$+{hour}$+{min}$+{sec}'

rename "s/^$pixel_modes_match$/${replace_datetime}_$+{stuff}/s" "$@"
rename "s/^$pixel_modes_match2$/${replace_datetime}_$+{stuff}/s" "$@"
rename "s/^$pixel_modes_match_screenshot$/${replace_datetime}_screenshot$+{stuff}/s" "$@"
rename "s/^$iphone_match$/${replace_datetime}_$+{stuff}/s" "$@"
